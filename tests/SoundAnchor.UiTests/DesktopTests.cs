...
namespace SoundAnchor.UiTests;

public sealed class DesktopTests
{
    private static string FindExecutable()
    {
        if (Environment.GetEnvironmentVariable("SOUNDANCHOR_EXE") is { } specified) return specified;
        var directory = new DirectoryInfo(AppContext.BaseDirectory);
        while (directory is not null && !File.Exists(Path.Combine(directory.FullName, "SoundAnchor.slnx"))) directory = directory.Parent;
        Assert.NotNull(directory);
        var config = AppContext.BaseDirectory.Contains("Release") ? "Release" : "Debug";
        return Path.Combine(directory.FullName, "src", "SoundAnchor.App", "bin", config, "net10.0-windows", "SoundAnchor.exe");
    }
    ...
    public void SelectApplyPauseRestorePersistAndCloseToTray()
    {
        ...
        var data = Path.Combine(Path.GetTempPath(), "SoundAnchor-UI", Guid.NewGuid().ToString("N"));
        ...
        {
            var results = Environment.GetEnvironmentVariable("SOUNDANCHOR_TEST_RESULTS") ?? Path.Combine(Directory.GetCurrentDirectory(), "TestResults");
